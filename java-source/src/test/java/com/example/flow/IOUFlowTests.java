...
public class IOUFlowTests {
    ...
    public void setup() {
        ...
        v = network.createPartyNode(null);
        // For real nodes this happens automatically, but we have to manually register the flow for tests.
        for (StartedMockNode node : ImmutableList.of(a, b, v)) {
            node.registerInitiatedFlow(RegularFlow.Acceptor.class);
        }
        network.runNetwork();
    }
    ...
    public void flowRejectsInvalidIOUs() throws Exception {
        // The RegularContract specifies that IOUs cannot have negative values.
        RegularFlow.Initiator flow = new RegularFlow.Initiator(-1, v.getInfo().getLegalIdentities().get(0), b.getInfo().getLegalIdentities().get(0));
    ...
    public void recordedTransactionHasNoInputsAndASingleOutputTheInputIOU() throws Exception {
        Integer iouValue = 1;
        RegularFlow.Initiator flow = new RegularFlow.Initiator(iouValue,v.getInfo().getLegalIdentities().get(0), b.getInfo().getLegalIdentities().get(0));
        CordaFuture<SignedTransaction> future = a.startFlow(flow);
        network.runNetwork();
        SignedTransaction signedTx = future.get();
